...
(reward_m_fix_2, tau_m_fix_2, knowledge_m_fix_2, exploration_m_fix_2) =perform_experiments(N_bandits, N_experiments, N_episodes, tau_fix_2, shock_prob_2, tau_strategy = "stable")

# Calculate exploration probability
exploration_prob_fix_2= np.count_nonzero(exploration_m_fix_2, axis=1)/N_episodes
...
(reward_m_acc_2, tau_m_acc_2, knowledge_m_acc_2, exploration_m_acc_2) =perform_experiments(N_bandits, N_experiments, N_episodes, tau_acc_2, shock_prob_2, tau_strategy= "accumulated resources")

# Calculate exploration probability
exploration_prob_acc_2= np.count_nonzero(exploration_m_acc_2, axis=1)/N_episodes
...
plt.plot(range(N_episodes), rewards_cum_avg_fix_2)
plt.plot(range(N_episodes), rewards_cum_avg_acc_2)
plt.xlabel('Periods')
...
plt.plot(range(N_episodes),knowledge_avg_over_time_fix_2)
plt.plot(range(N_episodes),knowledge_avg_over_time_acc_2)
plt.xlabel('Periods')
...
#Plot tau over time
plt.plot(range(N_episodes),tau_avg_fix_2)
plt.plot(range(N_episodes),tau_avg_acc_2)
plt.xlabel('Periods')
plt.ylabel('Tau')
plt.ylim(0,5)
plt.xlim(0,N_episodes)
